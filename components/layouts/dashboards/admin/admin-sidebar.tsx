...
import { Users, LogOut, ShieldCheck, ScrollText } from "lucide-react";
import { Button } from "@/components/ui/button";

const NAV = [
  { href: "/staff/admin/users", label: "Staff Accounts", icon: Users },
  { href: "/staff/admin/audit-log", label: "Audit Log", icon: ScrollText },
];

export default function AdminSidebar({ staffName }: { staffName: string }) {
    ...
    <aside className="border-border bg-card flex h-svh w-64 flex-col border-r">
      <div className="border-border flex items-center gap-2 border-b px-5 py-4">
        ...
        <div>
          ...
          <p className="text-primary text-xs font-medium">System Admin</p>
        </div>
      </div>
